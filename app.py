...
provinces_df = df5.filter(
    (~col("RegioS").isin("Gouda", "Nederland", "'s-Gravenhage (gemeente)"))
)
# Add column with absolute difference from Gouda's share
provinces_df = provinces_df.withColumn("diff", abs(col("share") - lit(gouda_share)))
# Get the province with the minimum difference
closest_province = provinces_df.orderBy("diff").limit(1)
# Get Gouda's row
gouda_df = df5.filter(col("RegioS") == "Gouda")
# Combine the two rows
result_df = gouda_df.unionByName(closest_province.select(df5.columns)).select("RegioS","share")
